...
def benchmark_tvm(mod, ctx):
    num_repeat = 1000
    ftimer = mod.module.time_evaluator("run", ctx, number=1, repeat=num_repeat)
    prof_res = np.array(ftimer().results)
    print("TVM FPS:", 1.0/np.mean(prof_res))
